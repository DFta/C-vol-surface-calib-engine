...
double x=x0; double f=0, df=0;
for(int i=0;i<maxit;++i){
f_df(x,f,df);
if(std::abs(df) < 1e-14) break;
double step = f/df;
x -= step;
if(std::abs(step) < tol*(1.0+std::abs(x))) return {x,i+1,true};
}
return {x,maxit,false};
}


inline Result brent(std::function<double(double)> f, double a, double b,
double tol=1e-10, int maxit=100){
double fa=f(a), fb=f(b);
if(fa*fb>0) throw std::invalid_argument("brent: root not bracketed");
double c=a, fc=fa; bool mflag=true; double s=b; double d=0;
for(int iter=1; iter<=maxit; ++iter){
if(std::abs(fc) < std::abs(fb)){ std::swap(a,b); std::swap(fa,fb); std::swap(c,b); std::swap(fc,fb);}
...
d=c; c=b; fc=fb;
double fs=f(s); a=((fb*fs)<0)? b:a; b=s; fb=fs;
if(a>b){ std::swap(a,b); std::swap(fa,fb);} else { fa=f(a);}
}
